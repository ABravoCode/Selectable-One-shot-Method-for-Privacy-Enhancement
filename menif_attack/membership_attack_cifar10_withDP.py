...
### 本代码是实现梯度成员推测攻击的入口(dp方案，在预测输出加入DP噪声，用于测试DP方案的预测准确率；在攻击实例的梯度加DP噪声，用于测试DP方案的攻击准确率)
...
np.save('../dataset/cifar10/x_train_cifar10_attack_withDP.npy',x_train_attack)
np.save('../dataset/cifar10/y_train_cifar10_attack_withDP.npy',y_train_attack)
np.save('../dataset/cifar10/x_test_cifar10_attack_withDP.npy',x_test_attack)
np.save('../dataset/cifar10/y_test_cifar10_attack_withDP.npy',y_test_attack)
...
train_feature_path = '../dataset/cifar10/x_train_cifar10_{}.npy'.format('attack_withDP')
train_lable_path = '../dataset/cifar10/y_train_cifar10_{}.npy'.format('attack_withDP')
test_feature_path = '../dataset/cifar10/x_test_cifar10_{}.npy'.format('attack_withDP')
test_lable_path = '../dataset/cifar10/y_test_cifar10_{}.npy'.format('attack_withDP')
...
#        layers_to_exploit=[],
        ...
        # 是否使用DP方案，并设置隐私预算（第一个为加入到预测输出的隐私预算，第二个为加入到攻击模型计算梯度的隐私预算）
...
# 将保存结果的数组转换为字符串并进行保存
info_save = '\n'.join(save_list)
with open('test_info_save_DP.txt', 'w') as f:     # 打开test.txt   如果文件不存在，创建该文件。
